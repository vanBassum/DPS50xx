...
private:
    ...
    RequestError fill(const ArgSpec& s)
    {
        ...
        case ArgType::Float:
        {
            // Same all-or-nothing parse as UInt32: strtof must consume the whole
            // token, so "12.5V" is a MalformedNumber rather than a silent 12.5.
            // A setpoint that was mistyped has to be refused, not rounded.
            char* end = nullptr;
